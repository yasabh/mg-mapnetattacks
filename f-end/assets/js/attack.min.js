function Attack(a, b) {
  this.map = a, this.svg = b, this.curZoom = this.map.getZoom(), this.beginPoint = {
    lat: 0,
    lng: 0
  }, this.endPoint = {
    lat: 0,
    lng: 0
  }, this.bp_px = null, this.ep_px = null, this.group = null, this.bp_circle = null, this.ep_circle = null, this.radius = 6 * this.curZoom / 3, this.plane = null, this.pos_plane = {
    x: 0,
    y: 0
  }, this.w_plane = 64, this.h_plane = 64, this.spos = 0, this.rot = 0, this.midPoint = {
    lat: 0,
    lng: 0
  }, this.mp_px = null, this.road = null, this.road_points = null, this.group_road = null, this.clipPath = null, this.clipPath_rect = null, this.planeColor = "white", this.roadColor = "white", this.beginColor = "blue", this.endColor = "red", this.isCleaning = !1, this.init = function(a, b) {
    this.beginPoint.lat = a.lat, this.beginPoint.lng = a.lng, this.bp_px = this.map.latLngToLayerPoint([a.lat, a.lng]), this.endPoint.lat = b.lat, this.endPoint.lng = b.lng, this.ep_px = this.map.latLngToLayerPoint([b.lat, b.lng]), this.pos_plane.x = this.bp_px.x, this.pos_plane.y = this.bp_px.y;
    var c = Math.random() < .5 ? -1 : 1,
      d = (a.lat + b.lat) / 2,
      e = (a.lng + b.lng) / 2;
    this.midPoint.lat = (a.lat + b.lat) / 2 + Math.random() * d * .25 * c, this.midPoint.lng = (a.lng + b.lng) / 2 + Math.random() * e * .25 * c, this.mp_px = this.map.latLngToLayerPoint([this.midPoint.lat, this.midPoint.lng]), this.group = this.svg.append("g"), this.road_points = [
    ...
    this.clipPath_rect = this.clipPath.append("rect").attr("x", f[0]).attr("y", f[1]).attr("width", f[2]).attr("height", f[3]), this.group_road = this.group.append("g"), this.group_road.attr("clip-path", "url(#aaa)"), this.road = this.group_road.append("path").datum(this.road_points).attr("stroke", this.roadColor).attr("stroke-width", 1).attr("stroke-linecap", "10 5").attr("fill", "none").attr("d", d3.line().curve(d3.curveBundle.beta(.5))), this.bp_circle = this.group.append("circle").attr("fill", this.beginColor), this.ep_circle = this.group.append("circle").attr("fill", this.endColor), this.load_plane()
    ...
    this.clipPath_rect.attr("x", a[0]).attr("y", a[1]).attr("width", a[2]).attr("height", a[3]), this.road_points = [
      ...
      [this.mp_px.x, this.mp_px.y],
      [this.ep_px.x, this.ep_px.y]
    ], this.road.datum(this.road_points).attr("d", d3.line().curve(d3.curveBundle.beta(.5))), this.bp_circle.attr("cx", this.bp_px.x).attr("cy", this.bp_px.y).attr("r", this.radius), this.ep_circle.attr("cx", this.ep_px.x).attr("cy", this.ep_px.y).attr("r", this.radius);
    var b = this.w_plane,
      c = this.h_plane,
      d = this.pos_plane.x,
      e = this.pos_plane.y,
